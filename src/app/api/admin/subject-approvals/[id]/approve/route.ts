...
/**
 * POST /api/admin/subject-approvals/[id]/approve {note?} — approve a PENDING
 * subject-approval as an admin override (§6.4 `POST .../[id]/approve|reject`).
 * Guarded UPDATE pending→approved, stamping decided_by/at + decision_note. The
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (recipient) {
    after(() => approvalDecision(recipient, dto.subject.name, "approved", note, { org_id: before.org_id }));
  }
