 ...
 * entry summarizes one broadcast: subject, when, recipient count, sent/failed
 * tallies. Newest first, paginated over the aggregated batches. org_id is
 * server-derived; RLS (`managed_org`) scopes email_log to the org.
 */
export async function GET(req: Request) {
  ...
  const { data, error } = await supabase
    .from("email_log")
    .select("batch_id, subject, status, created_at")
    .eq("org_id", orgId)
  ...
  for (const r of data ?? []) {
    ...
    if (r.created_at < b.sent_at) b.sent_at = r.created_at;
  }
