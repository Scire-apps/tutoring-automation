    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const profResp = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/tutor/profile`,
          {
        ...
        try {
          ...
          const apprResp = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/api/tutor/approvals`,
            {
              headers: {
        ...
        const oppResp = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/tutor/opportunities`,
          {
            headers: { Authorization: `Bearer ${session?.access_token ?? ""}` },
          }
        );
        ...
        if (typeof oppJson.tutor_status === "string")
          setTutorStatus(oppJson.tutor_status);
      } catch (err: any) {
  ...
  const handleApply = async (opportunityId: string) => {
    ...
    try {
      ...
      const resp = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/tutor/opportunities/${opportunityId}/apply`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${session?.access_token ?? ""}` },
        }
                        ...
                        >
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2">
                                  <div className="text-sm font-semibold text-gray-900 truncate">
                                    ...
                                    {opportunity.subject_type} • Grade{" "}
                                    {opportunity.subject_grade}
                                  </div>
                          ...
                          <td className="px-3 py-4">
                            <div className="text-sm text-gray-900 truncate">
                              ...
                              {(() => {
                                const g =
                                  opportunity.tutee?.grade ||
                                  (opportunity as any).tutee_grade ||
                                  (opportunity as any).grade;
                                return g ? ` (Grade ${g})` : "";
                              })()}
                            </div>
                            ...
                            <div className="flex items-center justify-end space-x-1">
                              {approvedSubjects.some((a) => {
                                ...
                                const typeOk =
                                  ...
                                  String(opportunity.subject_type || "");
                                const gradeOk =
                                  String(a.subject_grade || "") ===
                                  String(opportunity.subject_grade || "");
                                return (
                                  typeOk &&
                                  gradeOk &&
                                  base &&
                                  oppName.includes(base)
                                );
                              }) ? (
                                <button
                                  onClick={(e) => {
                                  ...
                                  disabled={(() => {
                                    ...
                                    const tuteeEmail = (opportunity.tutee
                                      ?.email ||
                                      (opportunity as any).tutee_email ||
                                      "") as string;
                                    const emailConflict =
                                      isHdsb(tutorEmail) &&
                                      isHdsb(tuteeEmail) &&
                                      baseLocal(tutorEmail) ===
                                        baseLocal(tuteeEmail);
                                    return (
                                      applyingTo === opportunity.id ||
                                      (tutorStatus &&
                                        tutorStatus.toLowerCase() !==
                                          "active") ||
                                      emailConflict
                                    );
                                  })()}
                                ...
                                >
                                  {(() => {
                                    ...
                                    const emailConflict =
                                      ...
                                      baseLocal(tutorEmail) ===
                                        baseLocal(tuteeEmail);
                                    if (applyingTo === opportunity.id)
                                      return "Applying...";
                                    if (
                                      tutorStatus &&
                                      tutorStatus.toLowerCase() !== "active"
                                    )
                                      return "Unavailable (Not Active)";
                                    if (emailConflict)
                                      return "Not Allowed (Same User)";
                                    return "Apply";
                                  })()}
                                </button>
                              ) : (
                                  ...
                                  <div>
                                    ...
                                    <div className="text-sm text-gray-700 space-y-1 rounded-xl border border-gray-100 p-3 shadow-sm">
                                      ...
                                      <p>
                                        ...
                                        {(
                                          opportunity.tutee?.email || "—"
                                        ).replace(
                                          /(^[^@\s]+)(\+(?:tutor|tutee))@([Hh][Dd][Ss][Bb]\.ca)$/,
                                          "$1@$3"
                                        )}
                                      </p>
                                      <p>
              ...
              <div className="flex items-start gap-4">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-50 text-blue-600 shadow-inner">
                  ...
                  >
                    <path
                      fillRule="evenodd"
                      d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm9.75-6a.75.75 0 0 1 .75.75v6a.75.75 0 0 1-1.5 0v-6A.75.75 0 0 1 12 6Zm0 12a1 1 0 1 0 0-2 1 1 0 0 0 0 2Z"
                      clipRule="evenodd"
                    />
                  </svg>
                </div>
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm leading-6 text-gray-700">
                    Once the student picks dates and times that work for them,
                    you may select a working date and time to schedule the
                    session.
                  </p>
                </div>
              </div>
