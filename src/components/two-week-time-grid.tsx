...
export function TwoWeekTimeGrid({ startHour = 7, endHour = 22, stepMinutes = 30, value, onChange, allowed, maxMinutesPerSession, className, singleDayOnly, singleContiguousRange }: TwoWeekTimeGridProps) {
  const dateCols = useMemo<string[]>(() => computeDateColumns(), []);
  ...
  const isAllowed = (date: string, slotIndex: number): boolean => {
  ...
  const setDateRanges = (date: string, ranges: Range[], lastStart?: string, lastEnd?: string): void => {
    ...
    if (singleContiguousRange && merged.length > 1) {
      ...
      const lastClose = lastEnd ?? last;
      const chosenIdx = merged.findIndex(r => last >= r.start && lastClose <= r.end);
      merged = chosenIdx >= 0 ? [merged[chosenIdx]] : [merged[0]];
    }
    let next: DateSelection = { ...value, [date]: merged };
    if (singleDayOnly) {
      // Clear other dates
      next = Object.keys(next).reduce<DateSelection>((acc, key) => {
  ...
  const toggleSlot = (date: string, slotIndex: number): void => {
  ...
  const onMouseDownCell = (date: string, slotIndex: number): void => {
    if (!isAllowed(date, slotIndex)) return;
    setIsDragging(true);
  ...
  const onMouseEnterCell = (date: string, slotIndex: number): void => {
    if (!isDragging) return;
    if (dragDateRef.current !== date) return;
    toggleSlot(date, slotIndex);
  };

  const onMouseUpGrid = (): void => {
    setIsDragging(false);
    dragDateRef.current = null;
  };

  const isSelected = (date: string, slotIndex: number): boolean => {
    const start = rows[slotIndex];
    const end = rows[slotIndex + 1] || rows[slotIndex];
