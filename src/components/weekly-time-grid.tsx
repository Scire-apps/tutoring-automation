...
export type WeeklySelection = {
  [day in DayKey]?: TimeRange[]; // HH:MM ranges
};

type AllowedMap = {
  [day in DayKey]?: TimeRange[]; // restrict selection
};
...
export function WeeklyTimeGrid({ startHour = 7, endHour = 22, stepMinutes = 30, value, onChange, allowed, maxMinutesPerSession, disallowedDays, className }: WeeklyTimeGridProps) {
  ...
  const isAllowed = (day: DayKey, slotIndex: number): boolean => {
  ...
  const getDayRanges = (day: DayKey): TimeRange[] => {
    return (value[day] || []).slice();
  };

  const setDayRanges = (day: DayKey, ranges: TimeRange[]): void => {
    const next: WeeklySelection = { ...value, [day]: mergeRanges(sortRanges(ranges)) };
    onChange(next);
  };

  const toggleSlot = (day: DayKey, slotIndex: number): void => {
  ...
  const onMouseDownCell = (day: DayKey, slotIndex: number): void => {
    if (!isAllowed(day, slotIndex)) return;
    setIsDragging(true);
  ...
  const onMouseEnterCell = (day: DayKey, slotIndex: number): void => {
  ...
  const isSelected = (day: DayKey, slotIndex: number): boolean => {
    const start = rows[slotIndex];
    const end = rows[slotIndex + 1] || rows[slotIndex];
